...
export async function POST(request: Request, { params }: { params: Promise<{ handoverId: string }> }) {
  ...
  if (handoverId !== DEMO_HANDOVER_ID || body.confirmation?.handoverId !== handoverId || !body.confirmation?.confirmedBy || !body.confirmation?.confirmedAt || !body.confirmation?.attestation) return NextResponse.json({ error: 'A complete carer confirmation is required before sharing.' }, { status: 400 });
  ...
  try {
    ...
    if (repository) {
      await repository.saveBundle(cannedDemoBundle);
